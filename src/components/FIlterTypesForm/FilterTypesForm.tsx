...
const FilterTypesForm: React.FC<IFilterTypesForm> = ({
  ...
  const handleChangeTypesFilter = (data: IData) => {
    const activeFiltersList = Object.entries(data).filter(([, state]) => state === true);

    const activeFiltersNames = activeFiltersList.map(([filterName]) => filterName);
      ...
      {filterByTypes.map((filterName: string) => {
        return (
          <div className={s.checkBoxContainer} key={filterName}>
            <input type="checkbox" name={filterName} placeholder={filterName} ref={register} />
